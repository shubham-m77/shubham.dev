    ...
    <div className="px-4 md:px-14 lg:px-20">
      ...
      <div className="flex flex-col mt-[37px]  ">
        {portfolioProjects.map((project,idx)=>(
          <div key={project.title} style={{
          ...
          }} className="sticky top-0 outline outline-[2px ] my-7 gap-10 px-6 md:px-12 md:pt-10 lg:px-0 pt-6 outline-gray-600/25 dark:outline-gray-300/25 after:z-10 rounded-2xl bg-black/10 dark:bg-white/10 backdrop-blur-sm relative after:content-[''] after:absolute after:inset-0 z-0 after:pointer-events-none  overflow-hidden ">
            ...
            {project.results.map((result)=>(
              <li key={result.title} className="text-gray-700 dark:text-gray-400  flex items-center text-sm gap-2"><CheckIcon size={4}/>{result.title}</li>
            ))}
            </ul>
